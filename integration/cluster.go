...
func (c *testCluster) AddManager() error {
	...
	if len(c.nodes) == 0 {
		...
		node, err := newTestNode(true, joinAddr)
		if err != nil {
			return err
		}
		n = node
...
func (c *testCluster) AddAgent() error {
	...
	node, err := newTestNode(false, joinAddr)
	if err != nil {
		return err
	}
	n = node
...
func (c *testCluster) SetNodeRole(id string, role api.NodeRole) error {
	...
	if role == api.NodeRoleWorker {
		// exclude demoted role early
		node.config.IsManager = false
	}
	...
	for i := 0; i < 5; i++ {
		...
		if _, err := c.api.UpdateNode(context.Background(), &api.UpdateNodeRequest{
			...
			if grpc.ErrorDesc(err) == "update out of sequence" || strings.Contains(grpc.ErrorDesc(err), "is closing") {
				continue
			}
			if role == api.NodeRoleWorker {
				node.config.IsManager = true
			}
			return err
		}
