...
// * one node is leader
// * number of workers and managers equals to exepcted
func pollClusterReady(t *testing.T, c *Cluster, numWorker, numManager int) {
	pollFunc := func() error {
...
func pollServiceReady(t *testing.T, c *Cluster, sid string) {
	pollFunc := func() error {
...
func newCluster(t *testing.T, numWorker, numManager int) *Cluster {
	cl := NewCluster()
